...
bool TextureUnit::readFile(const char * filename, int _channels)
{
	...
	int w, h, c, b;
	unsigned char * buffer = (unsigned char *) image_read(filename, &w, &h, &c, &b);
	if (!buffer)
	{
	...
	delete [] raw_texture;
	raw_texture = transform( buffer, w, h, c, _channels );
	width = w;
	height = h;
	channels = _channels;
...
unsigned char * TextureUnit::transform( const void * image, int w, int h, int bpp, int channels )
{
	int rowOffset = channels * w;
		
	// allocate memory
	unsigned char * data = new unsigned char[ w * h * channels ];
	const unsigned char * pixels = (const unsigned char *) image;
