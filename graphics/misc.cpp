...
string fileName(string input)
{
	...
	while ( i >= 0 )
	{
		if (input[i-1] == '.')
		{
			return input.substr(0, input.length() - l);
		}
		i--;
		l++;
	}
...
{
	ifstream inputTest(filename.c_str());
...
bool verifyCreateDir( const char * strPath )
{
	...
	if (stat(strPath, &st) == 0)
	{
		...
		{
			// directory exists, mark it as such
			return true;
		}
		else
		{
			// something other than a directory
			cerr << "The path: " << strPath << " is occupied by something other than a directory." << endl;
			return false;
		}
	}
	else
	{
		if (errno == ENOENT)
		{
			...
			{
				// error creating directory, fail
				cerr << "Error creating directory: " << strPath << endl;
				return false;
			}
			else
			{
		...
		{
			// error creating directory, fail
			cerr << "Error with directory: " << strPath << endl;
		}
	}
